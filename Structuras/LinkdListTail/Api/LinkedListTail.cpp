...
PYBIND11_MODULE(linkedlistTail, m) {
    m.doc() = "Python bindings for LinkedList with Tail"; 

    py::class_<LinkedList<int>>(m, "LinkedList")
        ...
        .def("booleanFind", &LinkedList<int>::booleanFind, py::arg("key"), "Devuelve si un elemento está en la lista")
        .def("erase", &LinkedList<int>::erase, py::arg("key"), "Elimina un nodo con un valor específico")
        .def("booleanEmpty", &LinkedList<int>::booleanEmpty, "Devuelve si la lista está vacía")
        .def("printList", &LinkedList<int>::printList, py::arg("os") = py::none(), "Imprime la lista");
}